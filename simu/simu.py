...
def write_unannot_splice(tx, all_exons, genome_fasta, out_prefix, junc_ref):
    '''
    ...
    seq, strand, ex_list = get_tx_seq(tx, all_exons, genome_fasta, control_fasta, wt_out=False)
    chrom = [x.chrom for x in all_exons if x['transcript_id'] == tx][0]

    exons = range(len(seq))
    possible_juncs = [ex for ex in itertools.combinations(exons, 2)]
    select1, select2, size = None, None, None
    ...
    # rerun tx seq, now that we know we have a valid tx we can write the wildtype out
    seq, strand, ex_list = get_tx_seq(tx, all_exons, genome_fasta, control_fasta)
    ...
    return size, '%d-%d' % (select1, select2)
