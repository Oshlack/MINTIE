...
class Fusion:
    ...
    def __init__(self, gene_name_1, gene_name_2, gene_blocks_1, gene_blocks_2, gene_sequence_1, gene_sequence_2):
        self.gene_name_1 = gene_name_1
        self.gene_name_2 = gene_name_2
        self.gene_blocks_1 = gene_blocks_1
        self.gene_blocks_2 = gene_blocks_2
...
def getBlocks(st_genes, gene):

    try:
        blocks = st_genes[gene][1]
    except KeyError:
        blocks = False
        #error(32, gene)
...
def mapSupertranscript(gene_1, gene_2, duplicates, found, not_found, total, st_genes):
    ...
    if gene_1 == "Not Found" or gene_2 == "Not Found":
        ...
        try:
            ...
            duplicates[gene_1+sep+gene_2] = True

        # Check if both genes are in the superTranscriptome
        sequence_1 = getSequence(st_genes, gene_1)
        sequence_2 = getSequence(st_genes, gene_2) if gene_1 != gene_2 and gene_2 != '' else ''
        ...
        if sequence_1 != False and sequence_2 != False:
            fusion = Fusion(gene_1, gene_2, blocks_1, blocks_2, sequence_1, sequence_2)
...
def createAnnotationFiles(fusions, st_genes, annotation_folder):
    ...
    for fusion in fusions.list:
        ...
        for exons, block in zip(gene_1_exons, gene_1_blocks):
            ...
            exons = fusion.gene_name_1 + sep + fusion.gene_name_2 + "\t" + str(exon_start) + "\t" + str(exon_end) + \
                    "\t" + str(exon_no) + "\t" + "0" + "\t" + "." + "\t" +  str(exon_start) + "\t" +  str(exon_end) + \
                    "\t" + RGB + "\n"
            exon_boundaries.write(exons)

        gene_line = fusion.gene_name_1 + sep + fusion.gene_name_2+"\t0\t"+str(exon_end + 1)+"\t"+fusion.gene_name_1+"\t0\t+\t0\t"+str(exon_end + 1)+"\t255,0,0\n"
        gene_boundaries.write(gene_line)

        gene_1_end = int(exon_end)
        gene_2_start = int(exon_end) + 1

        if fusion.gene_name_2 == '':
            continue

        gene_2_exons = st_genes[fusion.gene_name_2][1].split(",")
        gene_2_blocks = st_genes[fusion.gene_name_2][2].split(",")

        exon_no = 0

        for exons, block in zip(gene_2_exons, gene_2_blocks):

            #exon_no += 1
            exon_no = block

            exon_range = exons.split("-")
            exon_start = max(0, int(gene_1_end) + int(exon_range[0]) - 1)
            exon_end = int(gene_1_end) + int(exon_range[1])
        ...
        gene_line = fusion.gene_name_1 + sep + fusion.gene_name_2+"\t"+str(gene_2_start)+"\t"+str(exon_end)+"\t"+fusion.gene_name_2+"\t1\t+\t"+str(gene_2_start)+ \
                    "\t"+str(exon_end)+"\t0,186,255\n"
        gene_boundaries.write(gene_line)
